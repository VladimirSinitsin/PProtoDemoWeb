    ...
    wsServer.on('connection', (wsClient) => {
        ...
        const tcpSocket = net.connect({
            ...
            try {
                ...
                console.error(`[${connectionId}] Connection error, disconnecting. ${e}`);
                disconnect();
                return;
            }
            ...
            wsClient.on('message', (messageBuffer) => {
                try {
                    ...
                    console.log(`[${connectionId}] Message from client`, trimMessage(message));
                    sendMessage(tcpSocket, message);
                } catch (e) {
                    console.error(`[${connectionId}] Failed to process client message, disconnecting. ${e}`);
                    disconnect();
                }
            });
            wsClient.resume();
            ...
            while (tcpSocket.readable) {
                try {
                    ...
                    console.log(`[${connectionId}] Message from server`, trimMessage(message));
                    wsClient.send(message);
                } catch (e) {
                    console.error(`[${connectionId}] Failed to process server message, disconnecting. ${e}`);
                    disconnect();
                }
            }
        });
...
async function checkCompatibility(connectionId, tcpSocket) {
    ...
    if (response.command !== request.command) {
        throw new Error(`Protocol is not compatible, server response: ${response.command}`);
    }
